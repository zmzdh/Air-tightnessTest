...
namespace AudioActuatorCanTest.UserControls
{
    public partial class ReportControl : UserControl
    {
        ...
        private async Task ExportToCsv(string filePath)
        {
            ...
            csv.AppendLine("序号,测试时间,工单号,产品型号,产品代码,通道,测试结果,静态电流(mA),测试电压(V),失败原因,操作员,测试时长(s)");

            foreach (var record in allRecords)
            {
                var staticCurrent = record.StaticCurrent.HasValue ? record.StaticCurrent.Value.ToString("F1") : string.Empty;
                csv.AppendLine($"{record.Id},{record.TestTime:yyyy-MM-dd HH:mm:ss},{record.WorkOrder},{record.ProductModel},{record.ProductCode},{record.Channel},{record.Result},{staticCurrent},{record.TestVoltage:F1},\"{record.FailReason}\",{record.Operator},{record.TestDuration:F1}");
            }
