...
namespace LumbarMassageTest.Services
{
    public sealed class PressureModbusService : IDisposable
    {
        ...
        public async Task<double> ReadPressureKPaAsync(int channel, PressureChannelConfig? channelConfig, CancellationToken cancellationToken)
        {
            ...
            return ConvertRawToPressure(raw, channelConfig);
        }
        ...
        public async Task WriteOutputPressureAsync(int channel, double pressureKPa, PressureChannelConfig? channelConfig, CancellationToken cancellationToken)
        {
            ...
            ushort raw = ConvertPressureToOutputRaw(pressureKPa, channelConfig);
        ...
        private static double ConvertRawToPressure(int raw, PressureChannelConfig config)
        {
            if (config.FullScaleRaw == config.ZeroRaw)
            {
                return raw;
            }

            double ratio = (raw - config.ZeroRaw) / (double)(config.FullScaleRaw - config.ZeroRaw);
            return config.PressureZeroKPa + ratio * (config.PressureFullScaleKPa - config.PressureZeroKPa);
        }

        private static ushort ConvertPressureToOutputRaw(double pressureKPa, PressureChannelConfig config)
        {
            double min = config.Output4mAPressureKPa;
            double max = Math.Abs(config.Output20mAPressureKPa - min) < double.Epsilon
                ? min + 1
                : config.Output20mAPressureKPa;
            double ratio = (pressureKPa - min) / (max - min);
            ratio = Math.Clamp(ratio, 0, 1);
            return (ushort)Math.Round(ratio * ushort.MaxValue);
        }
